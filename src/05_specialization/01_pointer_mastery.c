...
void run_demo_arithmetic(void) {
  ...
  printf("  Note: Adding 1 to a pointer advances it by sizeof(type) bytes.\n\n");
}
...
void run_demo_array_pointers(void) {
  ...
  printf("  - ptr_to_array + 1:  %p (+%zu bytes, moves 1 whole array of 5 ints)\n\n",
...
void run_demo_multi_pointers(void) {
  ...
  ***ppp = 999;
  ...
  printf("  - New value of x:         %d ✓\n\n", x);
  printf("  Use case: Modifying pointers inside functions (pass by reference for pointers).\n\n");
}
