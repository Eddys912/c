...
Status parse_log_line(char *line, LogEntry *entry) {
  ...
  if (line[0] != '[') return ERR_PARSE_ERROR;

  char *end_bracket = strchr(line, ']');
  if (!end_bracket) return ERR_PARSE_ERROR;
  ...
  if (*ptr == '\0') return ERR_PARSE_ERROR;

  // Find colon separator for Level
  char *colon = strchr(ptr, ':');
  if (!colon) return ERR_PARSE_ERROR;

  int level_len = colon - ptr;
  if (level_len >= (int)sizeof(entry->level)) level_len = sizeof(entry->level) - 1;
...
void draw_bar(int count, int max_count, int max_bar_len) {
  ...
  if (count > 0 && blocks == 0) blocks = 1; // Show at least one for small > 0
...
void run_show_summary(void) {
  ...
  while (fgets(buffer, sizeof(buffer), file)) {
    if (parse_log_line(buffer, &entry) == SUCCESS) {
      total++;
      if (strcmp(entry.level, "INFO") == 0) info++;
      else if (strcmp(entry.level, "WARNING") == 0) warn++;
      else if (strcmp(entry.level, "ERROR") == 0) err++;
      else if (strcmp(entry.level, "CRITICAL") == 0) crit++;
    }
  }
  fclose(file);
  ...
  int max_val = info;
  if (warn > max_val) max_val = warn;
  if (err > max_val) max_val = err;
  if (crit > max_val) max_val = crit;
  ...
  draw_bar(info, max_val, 30); printf("\n");

  printf("  %-10s: %4d (%4.1f%%) | ", "WARNING", warn, (double)warn / total * 100);
  draw_bar(warn, max_val, 30); printf("\n");

  printf("  %-10s: %4d (%4.1f%%) | ", "ERROR", err, (double)err / total * 100);
  draw_bar(err, max_val, 30); printf("\n");

  printf("  %-10s: %4d (%4.1f%%) | ", "CRITICAL", crit, (double)crit / total * 100);
  draw_bar(crit, max_val, 30); printf("\n\n");
}

void run_filter_by_level(void) {
  char level_query[20];
  printf("\nEnter log level to filter (INFO, WARNING, ERROR, CRITICAL):\n> ");
  if (read_string(level_query, sizeof(level_query)) != SUCCESS) return;
...
void run_search_keyword(void) {
  ...
  if (read_string(keyword, sizeof(keyword)) != SUCCESS || strlen(keyword) == 0) return;
...
void run_temporal_analysis(void) {
  ...
  for (int i = 0; i < TOTAL_HOURS; i++) {
    if (hourly_counts[i] == 0) continue; // Skip zero hours for brevity

    printf("  %02d:00 - %02d:59 : %4d | ", i, i, hourly_counts[i]);
    draw_bar(hourly_counts[i], max_count, 40);

    if (i == peak_hour) printf(" ⚠ PEAK");
    printf("\n");
  }
...
void run_generate_dummy_log(void) {
  ...
  const char *levels[] = {"INFO", "INFO", "INFO", "INFO", "WARNING", "WARNING", "ERROR", "CRITICAL"};
  ...
  for (int i = 0; i < 300; i++) {
      ...
      if (strcmp(lvl, "INFO") == 0) {
          ...
          int err_type = rand() % 3;
          if (err_type == 0) msg = "Database connection timeout";
          else if (err_type == 1) msg = "Failed to write to disk";
          else msg = "Corrupted data packet received";
      } else {
          msg = "Kernel panic - syncing VFS";
      }

      fprintf(file, "[2026-01-03 %02d:%02d:%02d] %s: %s\n", hour, min, sec, lvl, msg);
  }
